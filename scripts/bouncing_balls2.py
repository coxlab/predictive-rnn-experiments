...
if __name__ == "__main__":
    ...
    #
    # N=200
    # dat=empty((N), dtype=ob('bouncing_balls_testing_data.mat'ject)
    # for i in xrange(N):
    #     dat[i]=bounce_vec(res=res, n=3, T=100)
    # data={}
    # data['Data']=dat
    ...
    specs = [(4000,'training'),(200,'validation'),(200,'testing')]

    for N,s in specs:
        data=zeros((N,T,res,res)).astype(float32)
        for i in xrange(N):
            data[i]=bounce_vec(res=res, n=3, T=T).reshape((T,res,res))
        f = open('/home/bill/Data/Bouncing_Balls/clip_set4/bouncing_balls_'+s+'_set.hkl','w')
        hkl.dump(data, f)
        f.close()
        scipy.io.savemat('/home/bill/Data/Bouncing_Balls/clip_set4/bouncing_balls_'+s+'_set.mat',{'clips': data})
