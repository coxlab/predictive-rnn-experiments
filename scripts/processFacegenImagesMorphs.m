function processFacegenImagesMorphs
...
out_dir=[base_dir 'morphs/images_processed/'];
mkdir(out_dir);

list=dir([base_dir 'morphs/images/']);


for i=1:length(list)
    if ~isempty(strfind(list(i).name,'.png'))
        try
            im=imread([base_dir 'morphs/images/' list(i).name]);
            im=rgb2gray(im);
        catch
            disp(['couldnt read: ' list(i).name]);
        end
